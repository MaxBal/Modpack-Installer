...
export interface Server {
    ...
    invocation: string;
    dockerImage: string;
    modpackLoader: 'forge' | 'neoforge' | 'fabric' | null;
    description: string;
    limits: {
        memory: number;
        swap: number;
...
const detectModpackLoader = (data: any): 'forge' | 'neoforge' | 'fabric' | null => {
    const value = `${data.modpack_loader || ''} ${data.name || ''} ${data.invocation || ''} ${data.docker_image || ''}`.toLowerCase();
...
export const rawDataToServerObject = ({ attributes: data }: FractalResponseData): Server => ({
    ...
    modpackLoader: detectModpackLoader(data),
    sftpDetails: {
